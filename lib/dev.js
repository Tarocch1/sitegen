...
const { mdToHtml, getAllMarkdownFile, Spinner, isIndex } = require('./utils');
const { sort } = require('./breadcrumb');
...
async function renderMdFiles(rootPath, mdFiles, clear = false) {
...
async function renderAllMarkdownFile(rootPath) {
  ...
  mdFiles.sort(sort);
  await renderMdFiles(rootPath, mdFiles);
}

function renderMarkdownFileFactory(rootPath, wss) {
  return async function (mdFilePath) {
    let mdFiles = [];
    if (isIndex(mdFilePath)) {
      const dirname = path.dirname(mdFilePath);
      const subMdFiles = await getAllMarkdownFile(dirname);
      mdFiles = subMdFiles.map((p) =>
        normalizePath(path.relative(rootPath, path.join(dirname, p)))
      );
    } else {
      mdFiles = [normalizePath(path.relative(rootPath, mdFilePath))];
    }
    await renderMdFiles(rootPath, mdFiles, true);
    wss.clients.forEach(function (client) {
      if (client.readyState === WebSocket.OPEN) {
        client.send(JSON.stringify(mdFiles.map((f) => `/${mdToHtml(f)}`)));
      }
    });
  };
}
...
function removeMarkdownFileFactory(rootPath, wss) {
  return async function (mdFilePath) {
    const mdFile = normalizePath(path.relative(rootPath, mdFilePath));
    const pathName = mdToHtml(mdFile);
