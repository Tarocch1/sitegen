...
const { isIndex, getGitLastUpdatedTimeStamp, Spinner } = require('./utils');
...
Handlebars.registerHelper('breadcrumb', renderBreadcrumb);
  ...
  if (template) {
    ...
    const title =
      parsed.matter.title !== undefined
        ...
        : config.config.title;
    if (isIndex(pathName)) {
      saveBreadcrumb(pathName, title);
    }
    const breadcrumbs = getBreadcrumbs(parsed, pathName);
    const data = {
...
function getBreadcrumbs(parsed, pathName) {
  const breadcrumbConfig =
    parsed.matter.breadcrumb !== undefined
      ? parsed.matter.breadcrumb
